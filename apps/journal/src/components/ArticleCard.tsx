  ...
  return (
    <div className="group journal-panel relative overflow-hidden p-8 transition-all duration-300 hover:-translate-y-1 hover:border-secondary/25 hover:shadow-[0_24px_60px_-45px_rgba(19,34,56,0.3)]">
      ...
      <div className="mb-5 flex items-center justify-between gap-3">
        <Badge variant="secondary" className="rounded-full border-0 bg-secondary/8 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-secondary">
          {article.type}
        </Badge>
      ...
      {article.keywords && article.keywords.length > 0 && (
        <div className="mb-5 flex flex-wrap gap-1.5">
          {article.keywords.slice(0, 3).map((kw) => (
            <span key={kw} className="rounded-full bg-[hsl(var(--highlight)/0.1)] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-[hsl(var(--highlight))]">
              {kw}
            </span>
          ))}
        </div>
      ...
      <p className="journal-body mb-8 line-clamp-4">
        {article.abstract}
      </p>
      ...
      <div className="flex items-center justify-between border-t border-border/90 pt-5">
         <div className="journal-meta">
            ...
            {article.doi && (
              <span className="ml-2 font-medium normal-case tracking-normal text-secondary/85">DOI: {article.doi}</span>
            )}
         </div>
         <div className="flex gap-2">
            <Button asChild variant="outline" size="sm" className="h-8 gap-1 rounded-full border-border bg-background/80 text-xs">
              <Link href={`/articles/${article.slug}`}>
            ...
            {article.pdfUrl ? (
              <Button asChild size="sm" className="h-8 gap-1 rounded-full text-xs">
              ...
              <Button size="sm" disabled className="h-8 gap-1 rounded-full text-xs">
                <Download className="h-3.5 w-3.5" />
                PDF
              </Button>
            )}
