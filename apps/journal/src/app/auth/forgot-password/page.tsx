...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const response = await apiRequest<{ message: string }>("/api/auth/forgot-password", "POST", {
        email,
      });
      ...
      <AuthCard>
        <div className="mb-8 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Enter your journal account email and we&apos;ll send you a reset link if the account exists.
          </p>
        </div>
