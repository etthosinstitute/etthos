  ...
  try {
    ...
    if (!isRazorpayEnabled()) {
      return NextResponse.json(
        {
          message: "Razorpay preview is configured in code but not enabled yet.",
        },
        { status: 503 },
      );
    }
    ...
    const message = error instanceof Error ? error.message : "Unable to initialise Razorpay preview.";
