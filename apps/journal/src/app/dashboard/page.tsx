...
function statusClasses(status: string) {
  ...
  if (status === "UNDER_REVIEW" || status === "ASSIGNED" || status === "ACCEPTED") {
...
export default function DashboardPage() {
  ...
  const [assignForms, setAssignForms] = useState<Record<string, { reviewerId: string; dueDate: string; editorNotes: string }>>({});
  const [reviewForms, setReviewForms] = useState<Record<string, { decision: string; content: string; confidentialComments: string }>>({});
  ...
  async function handleAssign(manuscriptId: string) {
    ...
    try {
      ...
      const response = await apiRequest<{ emailSent?: boolean; emailError?: string | null }>(
        `/api/manuscripts/${manuscriptId}/assign`,
        "POST",
        payload
      );
      setMessage({
  ...
  async function handleAssignmentResponse(assignmentId: string, status: "ACCEPTED" | "DECLINED") {
    try {
  ...
  async function handleSubmitReview(assignmentId: string, manuscriptId: string) {
  ...
  async function handleCreateReviewer() {
    if (!reviewerForm.firstName || !reviewerForm.lastName || !reviewerForm.email) {
      setMessage({
        tone: "info",
  ...
  async function handlePublish(manuscript: ManuscriptRecord) {
    ...
    try {
      ...
      const response = await apiRequest<{
        ...
        publishedDate: new Date(form.publishedDate || currentDate).toISOString(),
  ...
  async function handleChangePassword() {
    ...
    try {
      ...
      setMessage(null);
      const response = await apiRequest<{ message: string }>("/api/auth/change-password", "POST", passwordForm);
      setPasswordForm({
        currentPassword: "",
        newPassword: "",
        confirmPassword: "",
          ...
          <div className="space-y-8">
              ...
              <CardHeader className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <div className="journal-panel px-4 py-3">
                    <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground">Submissions</p>
                    <p className="mt-2 text-2xl font-semibold text-primary">{data.author.manuscripts.length}</p>
                  </div>
                  <div className="journal-panel px-4 py-3">
                    <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground">Pending Reviews</p>
                    <p className="mt-2 text-2xl font-semibold text-primary">{data.reviewer.pendingAssignments.length}</p>
                  </div>
                  <div className="journal-panel px-4 py-3">
                    <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground">Editorial Queue</p>
                    <p className="mt-2 text-2xl font-semibold text-primary">{data.editor.manuscripts.length}</p>
                  </div>
                </div>
              </CardHeader>
            </Card>
            ...
            <section className="grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
              <Card className="journal-shell">
                <CardHeader>
                  <CardTitle className="font-serif text-2xl text-primary">Author Submissions</CardTitle>
                  <CardDescription>Your manuscripts and their current editorial status.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {data.author.manuscripts.length === 0 ? (
                    <div className="journal-panel border-dashed p-6 text-sm text-muted-foreground">
                      No manuscripts submitted yet. <Link href="/submit" className="text-secondary underline-offset-4 hover:underline">Submit your first manuscript</Link>.
                    </div>
                  ) : (
                    data.author.manuscripts.map((manuscript) => (
                      <div key={manuscript.id} className="journal-panel p-5">
                        ...
                        {manuscript.abstract && (
                          <p className="mt-4 text-sm leading-6 text-muted-foreground">{manuscript.abstract}</p>
                        )}
                            ...
                            {manuscript.assignments.map((assignment) => (
                              <div key={assignment.id} className="rounded-xl border border-border/80 bg-card p-4 text-sm">
                                ...
                                <p className="mt-2 text-muted-foreground">
                                  Invited {formatDate(assignment.invitedAt)} • Due {formatDate(assignment.dueDate)}
                                </p>
                              </div>
                            ))}
                          </div>
              ...
              <Card className="journal-shell">
                <CardHeader>
                  <CardTitle className="font-serif text-2xl text-primary">Reviewer Workspace</CardTitle>
                  <CardDescription>Accept invitations, write reviews, and keep a record in the site.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {data.reviewer.pendingAssignments.length === 0 ? (
                    ...
                    data.reviewer.pendingAssignments.map((assignment) => {
                      ...
                      const reviewCommentCount = currentForm.content.trim().length;
                      const reviewReady = reviewCommentCount >= 10 && !!currentForm.decision;
                        ...
                        <div key={assignment.id} className="journal-panel p-5">
                            ...
                            {assignment.status === "ASSIGNED" ? (
                              <>
                                <Button
                                  ...
                                  disabled={busyKey === `respond-${assignment.id}-ACCEPTED`}
                                >
                                  {busyKey === `respond-${assignment.id}-ACCEPTED` ? "Saving..." : "Accept"}
                                </Button>
                                <Button
                                  type="button"
                                  size="sm"
                                  ...
                                  disabled={busyKey === `respond-${assignment.id}-DECLINED`}
                                >
                                  {busyKey === `respond-${assignment.id}-DECLINED` ? "Saving..." : "Decline"}
                                </Button>
                              </>
                            ) : null}
                          ...
                          {assignment.status === "ACCEPTED" ? (
                            <div className="mt-5 grid gap-4">
                              <div>
                                <label className="mb-2 block text-sm font-medium text-primary">Decision</label>
                                <select
                              ...
                              <div>
                                <div className="mb-2 flex items-center justify-between gap-3">
                                  <label className="block text-sm font-medium text-primary">Review comments</label>
                                ...
                                <p className="mt-2 text-xs text-muted-foreground">
                                  Add at least 10 characters so the editorial office receives a meaningful review note.
                                </p>
                              </div>
                              <div>
                                <label className="mb-2 block text-sm font-medium text-primary">Confidential comments for editors</label>
                                <Textarea
                                  rows={4}
                              ...
                              <Button
                                type="button"
                                onClick={() => handleSubmitReview(assignment.id, assignment.manuscript!.id)}
                                disabled={busyKey === `review-${assignment.id}` || !reviewReady}
                              >
                                {busyKey === `review-${assignment.id}` ? "Submitting..." : "Submit Review"}
                              </Button>
                              {!reviewReady ? (
                                <p className="text-xs text-muted-foreground">
                                  Select a decision and enter at least 10 characters before submitting.
                                </p>
                              ) : null}
                            </div>
                          ) : assignment.status === "SUBMITTED" ? (
                            <div className="mt-5 rounded-xl border border-emerald-600/15 bg-emerald-600/5 px-4 py-3 text-sm text-emerald-800">
                              Your review has already been submitted and recorded.
                            </div>
                          ) : assignment.status === "DECLINED" ? (
                            <div className="mt-5 rounded-xl border border-red-600/15 bg-red-600/5 px-4 py-3 text-sm text-red-700">
                              This invitation was declined, so the review form is no longer available.
                            </div>
                          ) : (
                  ...
                  {data.reviewer.submittedReviews.length > 0 && (
                    <div className="journal-panel p-5">
                      <h3 className="font-serif text-xl text-primary">Submitted Reviews</h3>
                      <div className="mt-4 space-y-3">
                        {data.reviewer.submittedReviews.map((review) => (
                          <div key={review.id} className="flex flex-col gap-2 rounded-xl border border-border bg-card p-4 text-sm md:flex-row md:items-center md:justify-between">
                            <div>
                              <p className="font-medium text-primary">{review.manuscript.title}</p>
                              <p className="text-muted-foreground">Submitted {formatDate(review.createdAt)}</p>
                            </div>
                            <span className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] ${statusClasses(review.decision)}`}>
            ...
            <Card className="journal-shell">
              <CardHeader>
                <CardTitle className="font-serif text-2xl text-primary">Account Security</CardTitle>
                <CardDescription>
                  Update your password from the journal dashboard. New passwords must be at least 8 characters long.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid gap-4 rounded-[1.5rem] border border-border/80 bg-[hsl(var(--paper)/0.84)] p-6 md:grid-cols-3">
                  <div>
                    <label className="mb-2 block text-sm font-medium text-primary">Current password</label>
                    <Input
                      type="password"
                  ...
                  <div>
                    <label className="mb-2 block text-sm font-medium text-primary">New password</label>
                    <Input
                      type="password"
                  ...
                  <div>
                    <label className="mb-2 block text-sm font-medium text-primary">Confirm new password</label>
                    <Input
                      type="password"
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  <Button
                    ...
                    disabled={busyKey === "change-password" || !passwordFormReady}
                  >
            ...
            {(data.user.role === "EDITOR" || data.user.role === "ADMIN") && (
                  ...
                  <div className="grid gap-4 rounded-[1.5rem] border border-border/80 bg-[hsl(var(--paper)/0.84)] p-6 md:grid-cols-2 xl:grid-cols-[0.8fr_0.8fr_1.2fr_auto]">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setReviewerForm((prev) => ({ ...prev, firstName: e.target.value }))
                        }
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setReviewerForm((prev) => ({ ...prev, lastName: e.target.value }))
                        }
                        placeholder="Verma"
                      />
                    </div>
                    <div>
                      <label className="mb-2 block text-sm font-medium text-primary">Official email</label>
                      <Input
                        type="email"
                        value={reviewerForm.email}
                        onChange={(e) =>
                          setReviewerForm((prev) => ({ ...prev, email: e.target.value }))
                        }
              ...
              <Card className="journal-shell">
                <CardHeader>
                  <CardTitle className="font-serif text-2xl text-primary">Editorial Assignment Board</CardTitle>
                  <CardDescription>
                    Assign reviewers, monitor review status, and keep the workflow stored in the journal site.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  ...
                  {data.editor.manuscripts.map((manuscript) => {
                    ...
                    return (
                      <div key={manuscript.id} className="rounded-[1.5rem] border border-border/80 bg-[hsl(var(--paper)/0.82)] p-6 shadow-[0_18px_45px_-42px_rgba(19,34,56,0.26)]">
                        ...
                        <div className="mt-6 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                          <div className="grid gap-4">
                            <div>
                              <label className="mb-2 block text-sm font-medium text-primary">Assign reviewer</label>
                              <select
                                className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm shadow-sm"
                              ...
                              <p className="mt-2 text-xs text-muted-foreground">
                                Choose one reviewer account to send an invitation and due date.
                              </p>
                            </div>
                            <div>
                              <label className="mb-2 block text-sm font-medium text-primary">Editorial notes</label>
                              <Textarea
                                rows={4}
                              ...
                              />
                              <p className="mt-2 text-xs text-muted-foreground">
                                These notes are included with the reviewer invitation email.
                              </p>
                            </div>
                          </div>
                          <div className="grid gap-4">
                            <div>
                              <label className="mb-2 block text-sm font-medium text-primary">Due date</label>
                              <Input
                                type="date"
                            ...
                            >
                              {busyKey === `assign-${manuscript.id}` ? "Assigning..." : "Assign Reviewer"}
                            </Button>
                            {!assignForm.reviewerId ? (
                              <p className="text-xs text-muted-foreground">
                        ...
                        {manuscript.status === "ACCEPTED" && !manuscript.publishedArticleId && (
                          <div className="mt-6 rounded-[1.35rem] border border-secondary/20 bg-secondary/5 p-5">
                            <div className="mb-4">
                              ...
                              <h4 className="mt-2 font-serif text-xl text-primary">
                                Publish accepted manuscript to the journal archive
                              </h4>
                              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                                This will create or reuse the selected volume and issue, generate a public article record,
                                and move the manuscript into the published archive.
                              </p>
                            </div>
                              ...
                              return (
                                <div className="grid gap-4">
                                  <div className="grid gap-4 md:grid-cols-2">
                                    <div>
                                      <label className="mb-2 block text-sm font-medium text-primary">Article title</label>
                                      <Input
                                        value={form.articleTitle}
                                        onChange={(e) =>
                                          setPublishForms((prev) => ({
                                            ...prev,
                                            [manuscript.id]: { ...form, articleTitle: e.target.value },
                                          }))
                                        }
                                      />
                                      <p className="mt-2 text-xs text-muted-foreground">
                                        This is the public title that will appear in the issue archive.
                                      </p>
                                    </div>
                                    <div>
                                      <label className="mb-2 block text-sm font-medium text-primary">Article type</label>
                                      <select
                                        className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm shadow-sm"
                                        ...
                                        onChange={(e) =>
                                          setPublishForms((prev) => ({
                                            ...prev,
                                            [manuscript.id]: { ...form, articleType: e.target.value },
                                          }))
                                        }
                                      >
                                        ...
                                        <option value="LETTER_TO_EDITOR">Letter to the Editor</option>
                                  ...
                                  <div className="grid gap-4 md:grid-cols-4">
                                    <div>
                                      <label className="mb-2 block text-sm font-medium text-primary">Volume</label>
                                      <Input
                                        type="number"
                                        min="1"
                                        ...
                                        onChange={(e) =>
                                          setPublishForms((prev) => ({
                                            ...prev,
                                            [manuscript.id]: { ...form, volumeNumber: e.target.value },
                                          }))
                                        }
                                      />
                                    </div>
                                    <div>
                                      <label className="mb-2 block text-sm font-medium text-primary">Year</label>
                                      <Input
                                        type="number"
                                        min="1900"
                                        ...
                                        onChange={(e) =>
                                          setPublishForms((prev) => ({
                                            ...prev,
                                            [manuscript.id]: { ...form, volumeYear: e.target.value },
                                          }))
                                        }
                                      />
                                    </div>
                                    <div>
                                      <label className="mb-2 block text-sm font-medium text-primary">Issue</label>
                                      <Input
                                        type="number"
                                        min="1"
                                        ...
                                        onChange={(e) =>
                                          setPublishForms((prev) => ({
                                            ...prev,
                                            [manuscript.id]: { ...form, issueNumber: e.target.value },
                                          }))
                                        }
                                      />
                                    </div>
                                    <div>
                                      <label className="mb-2 block text-sm font-medium text-primary">Published date</label>
                                      <Input
                                        type="date"
                                        ...
                                        onChange={(e) =>
                                          setPublishForms((prev) => ({
                                            ...prev,
                                            [manuscript.id]: { ...form, publishedDate: e.target.value },
                                          }))
                                        }
                                      />
                                    </div>
                                  ...
                                  <div className="grid gap-4 md:grid-cols-2">
                                    <div>
                                      <label className="mb-2 block text-sm font-medium text-primary">Issue title</label>
                                      <Input
                                        value={form.issueTitle}
                                        onChange={(e) =>
                                          setPublishForms((prev) => ({
                                            ...prev,
                                            [manuscript.id]: { ...form, issueTitle: e.target.value },
                                          }))
                                        }
                                    ...
                                    <div>
                                      <label className="mb-2 block text-sm font-medium text-primary">DOI</label>
                                      <Input
                                        value={form.doi}
                                        onChange={(e) =>
                                          setPublishForms((prev) => ({
                                            ...prev,
                                            [manuscript.id]: { ...form, doi: e.target.value },
                                          }))
                                        }
                                  ...
                                  <div>
                                    <label className="mb-2 block text-sm font-medium text-primary">Issue description</label>
                                    <Textarea
                                      rows={3}
                                      ...
                                      onChange={(e) =>
                                        setPublishForms((prev) => ({
                                          ...prev,
                                          [manuscript.id]: { ...form, issueDescription: e.target.value },
                                        }))
                                      }
                                  ...
                                  <div className="grid gap-4 md:grid-cols-2">
                                    <div>
                                      <label className="mb-2 block text-sm font-medium text-primary">Page start</label>
                                      <Input
                                        type="number"
                                        min="1"
                                        ...
                                        onChange={(e) =>
                                          setPublishForms((prev) => ({
                                            ...prev,
                                            [manuscript.id]: { ...form, pageStart: e.target.value },
                                          }))
                                        }
                                    ...
                                    <div>
                                      <label className="mb-2 block text-sm font-medium text-primary">Page end</label>
                                      <Input
                                        type="number"
                                        min="1"
                                        value={form.pageEnd}
                                        onChange={(e) =>
                                          setPublishForms((prev) => ({
                                            ...prev,
                                            [manuscript.id]: { ...form, pageEnd: e.target.value },
                                          }))
                                        }
                                        placeholder="Optional"
                                      />
                                  ...
                                  <div className="flex flex-wrap gap-3">
                                    <Button
                                      type="button"
                                      onClick={() => handlePublish(manuscript)}
                                      disabled={busyKey === `publish-${manuscript.id}` || !form.articleTitle.trim()}
                                    >
                                      {busyKey === `publish-${manuscript.id}` ? "Publishing..." : "Publish to Issue"}
                                    </Button>
                                    {manuscript.fileUrl ? (
                        ...
                        {manuscript.status === "PUBLISHED" && (
                          <div className="mt-6 rounded-[1.25rem] border border-emerald-600/20 bg-emerald-600/8 px-4 py-3 text-sm text-emerald-800">
                            This manuscript is already published in the journal archive.
                          </div>
                        )}
                      </div>
                    );
              ...
              <Card className="journal-shell">
                <CardHeader>
                  <CardTitle className="font-serif text-2xl text-primary">Editorial Activity Log</CardTitle>
                  <CardDescription>
                    A running record of editorial and reviewer workflow actions stored in the journal system.
                  </CardDescription>
                </CardHeader>
                    ...
                    data.editor.auditLogs.map((entry) => (
                      <div key={entry.id} className="journal-panel p-4">
                        <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                          <div>
                            <p className="text-sm font-medium text-primary">{entry.summary}</p>
