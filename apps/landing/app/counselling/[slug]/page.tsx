...
export async function generateStaticParams() {
  return counsellingServices.map((service) => ({
    slug: service.slug,
  }));
}

const CounsellingServicePage = async ({ params }: PageProps) => {
  const { slug } = await params;
  const service = counsellingServices.find((s) => s.slug === slug);
          ...
          <div className="grid lg:grid-cols-[1fr_380px] gap-12 lg:gap-20">
            ...
            <div className="space-y-8 lg:sticky lg:top-28">
              <div className="bg-gradient-to-br from-brand-cyan to-brand-cobalt rounded-2xl p-8 text-white shadow-2xl shadow-brand-cyan/20">
                ...
                <p className="text-white/80 text-sm mb-6">Book your counselling session today.</p>
                ...
                <div className="mt-6 pt-6 border-t border-white/20 text-xs text-white/60">
                  Confidential support with licensed professionals.
                </div>
              </div>
            </div>
          </div>
...
export default CounsellingServicePage;
